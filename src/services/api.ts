...
export const fetchPopularPosts = async (): Promise<BlogPost[]> => {
  try {
    const response = await api.get(`/posts-popular/?site_domain=${SITE_DOMAIN}`);
    return response.data.results;
  } catch (error) {
...
export const fetchPostsByCategory = async (categorySlug: string, page = 1): Promise<{ posts: BlogPost[], totalPages: number }> => {
  try {
    const response = await api.get(`/posts/?site_domain=${SITE_DOMAIN}&category=${categorySlug}&page=${page}`);
    return {
...
export const fetchAllPosts = async (page = 1): Promise<{ posts: BlogPost[], totalPages: number }> => {
  try {
    const response = await api.get(`/posts/?site_domain=${SITE_DOMAIN}&page=${page}`);
    return {
      posts: response.data.results,
      totalPages: Math.ceil(response.data.count / 10), // Assuming 10 posts per page
    };
...
export const searchPosts = async (query: string): Promise<BlogPost[]> => {
  try {
    const response = await api.get(`/posts/?site_domain=${SITE_DOMAIN}&search=${query}`);
    return response.data.results;
  } catch (error) {
